    ...
    return (
        <div className={`group relative overflow-hidden rounded-2xl transition-all duration-500 hover:scale-105 ${featured ? 'bg-gradient-to-br from-yellow-500/20 to-orange-500/20 border-2 border-yellow-400/30' : 'bg-black/20 border border-white/10'
            ...
            <div className="p-6">
                ...
                <Link
                    href={`/en/products/${product.id || product._id}`}
...
export default function CatalogPage() {
    ...
    const fetchProducts = async () => {
        try {
            ...
            if (response.ok) {
                ...
                // Extract unique categories (prioritize English names)
                const uniqueCategories = Array.from(new Set(data.map((p: ProductItem) => p.categoryEn || p.category).filter(Boolean))) as string[];
    ...
    const filteredProducts = selectedCategory === 'all'
        ...
        : products.filter(p => p.published && (p.categoryEn === selectedCategory || p.category === selectedCategory));
                        ...
                        <>
                            ...
                            {filteredProducts.length === 0 && !loading && (
                                <div className="text-center py-20">
                                    <h3 className="text-2xl font-bold text-white/60 mb-4">No Products Found</h3>
                                    <p className="text-white/40">No products are currently available in this category</p>
                                </div>
                            )}
                        </>
                    )}
